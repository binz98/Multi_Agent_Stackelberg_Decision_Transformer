...
class ActorCritic(nn.Module):
    ...
    def __init__(self, n_embed, n_head, n_agent, action_dim, n_block, action_type='Discrete', device="CPU"):
        ...
        self.action_ln = nn.LayerNorm(n_embed)

        self.obs_ln1 = nn.LayerNorm(n_embed)
        self.obs_ln2 = nn.LayerNorm(n_embed)
        self.seq_ln = nn.LayerNorm(n_embed)
        self.query_ln = nn.LayerNorm(n_embed)
        ...
        self.gru = nn.GRU(n_embed, n_embed, num_layers=2, batch_first=True)
    ...
    def forward(self, action, obs_emb):
        ...
        # state_rep = obs_emb[:,0:1,:]
        # obs_rep = obs_emb[:,1:,:]
        action_emb = self.action_encoder(action)
        action_rep = action_emb

        # seq = self.pos_embed(torch.cat([state_rep, action_rep], dim=1)) # v1
        seq = torch.cat([state_rep, action_rep], dim=1) # v3
        x, _ = self.gru(self.obs_ln1(seq))
        y = x[:,:-1,:]
        y = y + obs_rep
        y = y + self.out_proj(y)    #v3_6
        x = self.seq_ln(y)
        logit = self.action_head(x)
        v_loc = self.value_head(x)
        return logit, v_loc
