...
class TransformerPolicy:
    """
    STEER Policy  class. Wraps actor and critic networks to compute actions and value function predictions.
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if self.algorithm_name == "steer":
            from steer.algorithms.steer.algorithm.steer import Steer as Steer
        elif self.algorithm_name == "steer_gru":
            from steer.algorithms.steer.algorithm.steer_gru import Steer as Steer
        else:
            raise NotImplementedError

        self.transformer = Steer(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                               ...
                               share_actor=args.share_actor, add_state_token=args.add_state_token)
    ...
    def get_values(self, cent_obs, obs, actions, rnn_states_critic, masks, available_actions=None):
        """
        ...
        cent_obs = cent_obs.reshape(-1, self.num_agents, self.share_obs_dim)
        obs = obs.reshape(-1, self.num_agents, self.obs_dim)
        actions = actions.reshape(-1, self.num_agents, actions.shape[-1])
        if available_actions is not None:
            available_actions = available_actions.reshape(-1, self.num_agents, self.act_dim)

        values = self.transformer.get_values(cent_obs, obs, actions, available_actions)
