...
class GlobalState(ObservationType):
    """Observe the kinematics of all vehicles."""
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        super().__init__(env)
        self.features = features or self.FEATURES
        self.vehicles_count = 2*len(self.env.controlled_vehicles)+1
        self.features_range = features_range
        self.normalize = normalize
        self.clip = clip
        self.observe_intentions = observe_intentions
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add controlled vehicle
        df = pd.DataFrame.from_records([v.to_dict() for v in self.env.controlled_vehicles])[self.features]
        df = df.append(pd.DataFrame.from_records([v.to_dict() for v in self.env.road.vehicles if v not in self.env.controlled_vehicles])[self.features], ignore_index=True)
        # Normalize and clip
        if self.normalize:
            df = self.normalize_obs(df)
        # Fill missing rows
...
class GlobalState2(ObservationType):
    """Observe the kinematics of all vehicles."""
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add nearby traffic
        # sort = self.order == "sorted"
        for cv in self.env.controlled_vehicles:
            close_vehicles = self.env.road.close_vehicles_to(cv,
                                                            self.env.PERCEPTION_DISTANCE,
                                                            count=self.vehicles_count - 1,
                                                            see_behind=self.see_behind)
            if close_vehicles:
                obs_origion = pd.DataFrame.from_records([v.to_dict(cv, observe_intentions=self.observe_intentions) 
                                                        for v in close_vehicles[-self.vehicles_count + 1:]])[self.features]
                if obs_origion.shape[0] < self.vehicles_count-1:
                    rows = np.zeros((self.vehicles_count - obs_origion.shape[0] - 1, len(self.features)))
                    obs_origion = obs_origion.append(pd.DataFrame(data=rows, columns=self.features), ignore_index=True)                   
                df = df.append(obs_origion, ignore_index=True)
            else:
                rows = np.zeros((self.vehicles_count - 1, len(self.features)))
                df = df.append(pd.DataFrame(data=rows, columns=self.features), ignore_index=True)
...
class GlobalState3(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        for vehicle in self.env.controlled_vehicles:
            one_df = pd.DataFrame.from_records([vehicle.to_dict()])[self.features]
            # Add nearby traffic
            # sort = self.order == "sorted"
            ...
            if close_vehicles:
                ...
                one_df = one_df.append(pd.DataFrame.from_records(
                    [v.to_dict(origin, observe_intentions=self.observe_intentions)
                    for v in close_vehicles[-self.vehicles_count + 1:]])[self.features],
                            ignore_index=True)
            # Normalize and clip
            if self.normalize:
                one_df = self.normalize_obs(one_df)
            # Fill missing rows
            if one_df.shape[0] < self.vehicles_count:
                rows = np.zeros((self.vehicles_count - one_df.shape[0], len(self.features)))
                one_df = one_df.append(pd.DataFrame(data=rows, columns=self.features), ignore_index=True)
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "GlobalState":
        return GlobalState(env, **config)
    elif config["type"] == "GlobalState2":
        return GlobalState2(env, **config)
    elif config["type"] == "GlobalState3":
        return GlobalState3(env, **config)
    else:
