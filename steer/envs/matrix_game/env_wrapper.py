...
from envs.matrix_game.matrix_env import MatrixGame 
from envs.matrix_game.nstep_matrix import NStepMatrixGame
...
class SubprocVecEnv(object):
    def __init__(self, all_args):
        ...
        if all_args.env_name == 'matrix':
            self.env_list = [MatrixGame(all_args) for _ in range(all_args.n_rollout_threads)]
        elif all_args.env_name == 'nstep_matrix':
            self.env_list = [NStepMatrixGame(all_args) for _ in range(all_args.n_rollout_threads)]
    ...
    def step(self, actions):
        ...
        return np.stack(obs), np.expand_dims(np.stack(rews), axis=-1), np.stack(dones), infos
...
class DummyVecEnv(object):
    def __init__(self, all_args):
        ...
        """
        if all_args.env_name == 'matrix':
            self.env_list = [MatrixGame(all_args) for _ in range(all_args.n_eval_rollout_threads)]
        elif all_args.env_name == 'nstep_matrix':
            self.env_list = [NStepMatrixGame(all_args) for _ in range(all_args.n_eval_rollout_threads)]
        self.num_envs = all_args.n_eval_rollout_threads
