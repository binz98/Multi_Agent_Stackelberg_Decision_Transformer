...
from envs.ma_mujoco_state.multiagent_mujoco.mujoco_multi import MujocoMulti as MujocoMultiState
        ...
        def init_env():
            if all_args.env_name == "mujoco":
                env_args = {"scenario": all_args.scenario,
                            ...
                            "episode_limit": 1000}
                if all_args.agent_obsk == 2:
                    env = MujocoMultiState(env_args=env_args)
                else:
...
if __name__ == "__main__":
    # main(sys.argv[1:])
    if len(sys.argv[1:]) == 0:
        argv = ['--env_name', 'mujoco', '--scenario', 'Ant-v2', '--agent_conf', '8x1', '--agent_obsk', '2',\
            '--algorithm_name', 'steer_state_v3', '--lr', '5e-5', '--critic_lr', '5e-5', '--faulty_node', '-1', '--eval_faulty_node', '-1',\
            '--experiment_name', 'debug', '--entropy_coef', '0.001', '--max_grad_norm', '0.5', '--eval_episodes', '5',\
            '--seed', '1', '--n_training_threads', '16', '--n_rollout_threads','40', '--clip_param', '0.05',\
            '--num_mini_batch', '40', '--episode_length', '100', '--num_env_steps', '10000000','--ppo_epoch', '10', '--eval_interval', '25', \
            '--use_wandb', 'False', '--use_eval', '--use_value_active_masks',  '--running_id', '1', '--add_center_xy', '--use_state_agent',\
            '--use_policy_active_masks' ] # , '--cal_reg']
    else:
        argv = sys.argv[1:]
        print(argv)
    main(argv)
